...
namespace vl53l1x {
    ...
    VL53L1X::VL53L1X(uint32_t update_interval,i2c::I2CBus *i2c) : PollingComponent(update_interval)
...
//    bus->write(value);
//    last_status = bus->endTransmission(true);
    uint8_t data[2];
    data[0] = (uint8_t)(value);
    data[1] = 0x0;
    last_status = write_register16(reg, data, 1, true);
    }
    ...
    data[2] = 0x0;
    last_status = write_register16(reg, data, 2, true);
    }
...
//    last_status = bus->endTransmission(true);
    ...
    uint8_t VL53L1X::readReg(regAddr reg)
    {
        ...
        uint8_t data[2]; 
        data[1] = 0;
        last_status = read_register16(reg, data, 1, true);
        value = data[0]
        return value;
    }
    ...
    uint16_t VL53L1X::readReg16Bit(uint16_t reg)
    {
        ...
        uint8_t data[3]; 
        data[2] = 0;
        last_status = read_register16(reg, data, 2, true);

        value  = (uint16_t)data[0] << 8; // value high byte
    ...
    uint32_t VL53L1X::readReg32Bit(uint16_t reg)
    {
        ...
        uint8_t data[5]; 
        data[4] = 0;
        last_status = read_register16(reg, data, 4, true);
    ...
    void VL53L1X::readResults()
    {
        ...
        buffer[17] = 0;
        uint8_t unused_byte;
        last_status = read_register16(RESULT__RANGE_STATUS,buffer,17, true );
        results.range_status = buffer[0];
        unused_byte = buffer[1]; // report_status: not used
        ...
        unused_byte =  buffer[5]; // peak_signal_count_rate_mcps_sd0: not used
        unused_byte =  buffer[6];
        results.ambient_count_rate_mcps_sd0  = (uint16_t)buffer[7] << 8; // high byte
        results.ambient_count_rate_mcps_sd0 |=           buffer[8];      // low byte
        unused_byte =  buffer[9];; // sigma_sd0: not used
        unused_byte =  buffer[10];
        unused_byte =  buffer[11]; // phase_sd0: not used
        unused_byte =  buffer[12];
    ...
    void VL53L1X::setup () {
        ...
        setDistanceMode(VL53L1X::Medium);
