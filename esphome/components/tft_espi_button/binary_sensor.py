...
TFT_eSPI_Button = tft_espi_widgets_ns.class_(
    "TFT_eSPI_ESPHome_Button", cg.Component, binary_sensor.BinarySensor
)
...
async def to_code(config):
    ...
    cg.add(
        var.configure(
            espi,
            position_conf[CONF_X_POS],
            position_conf[CONF_Y_POS],
            position_conf[CONF_WIDTH],
            position_conf[CONF_HEIGHT],
        )
    )
